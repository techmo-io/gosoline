...
func (k *kernel) runModule(ctx context.Context, name string, ms *ModuleState) (moduleErr error) {
	defer k.logger.Info("stopped %s module %s", ms.Config.Type, name)

	k.logger.Info("running %s module %s in stage %d", ms.Config.Type, name, ms.Config.Stage)
	...
	defer func(ms *ModuleState) {
		...
		if ms.Err != nil {
			k.logger.Error(ms.Err, "error running %s module %s", ms.Config.Type, name)
		}
